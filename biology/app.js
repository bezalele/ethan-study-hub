...
function course(id){const u=unit(id);crumbs(u?[['Course','course'],[u.n]]:[['Course']]);if(!u)return heading('THE COURSE','From living systems to living cells','Begin with foundations, or jump to your class topic.')+unitCards(true);return heading(u.n.toUpperCase(),u.title,u.desc)+`<div class="controls">${btn('Practice this unit','practice/'+u.id)}${btn('Unit check','check/'+u.id,true)}${btn('Vocabulary','flashcards/'+u.id,true)}</div><div class="grid">${BIO_LESSONS.filter(l=>l.unit===u.id).map(lessonCard).join('')}</div>${adjacentUnitNav(u.id)}`;}
...
function missing(){return heading('NOT FOUND','Let’s find your next lesson')+btn('Open the course','course');}
...
function route(){session=null;document.body.classList.remove('nav-open');$('#menu').setAttribute('aria-expanded','false');let [page='home',id]=location.hash.slice(1).split('/');if(!page)page='home';document.body.classList.toggle('home-page',page==='home');$('#nav').innerHTML=[['home','⌂','Home'],['course','◫','Course'],['practice','✎','Practice'],['flashcards/all','▤','Vocabulary'],['review','↺','Review'],['progress','▥','Progress']].map(([p,icon,t])=>`<a href="#${p}" class="${page===p.split('/')[0]||p==='course'&&['unit','lesson'].includes(page)?'active':''}"><span aria-hidden="true">${icon}</span> &nbsp; ${t}</a>`).join('');const main=$('#main');if(page==='flashcards'){flashcards(id);}else{main.innerHTML=page==='home'?home():page==='course'||page==='unit'?course(id):page==='lesson'?showLesson(id):page==='practice'||page==='check'?practiceHub(id,page==='check'):page==='review'?review():page==='progress'?progress():page==='about'?about():page==='settings'?settings():page==='search'?searchResults(id):missing();}
makeInternalLinksPreviewSafe();if(page==='lesson'&&lesson(id)){mountVisual(lesson(id).visual,id);$('#thinking').oninput=e=>{state.notes[id]=e.target.value;save();};$('#explored').onclick=e=>{if(!state.explored.includes(id))state.explored.push(id);save();e.target.textContent='✓ Lesson explored';};document.querySelectorAll('[data-scroll]').forEach(a=>a.onclick=e=>{e.preventDefault();document.getElementById(a.dataset.scroll).scrollIntoView({behavior:'smooth'});});}if($('#start-session'))$('#start-session').onclick=()=>startSession(id,page==='check');if(page==='settings')bindSettings();document.title=(page==='lesson'&&lesson(id)?lesson(id).title+' · ':'')+'Ethan’s Biology Field Guide';window.scrollTo(0,0);main.focus({preventScroll:true});}
