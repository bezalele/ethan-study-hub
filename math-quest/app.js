...
function course(query = "") {
  ...
  main.innerHTML = `<h1 class="page-title">${query ? "Find your next step" : "Your Algebra 1 course"}</h1><p class="intro">Explore the whole course. Choose the topic you’re learning at school, or revisit an earlier idea.</p>${query ? `<p>Results for “${esc(query)}”</p>` : ""}${list.length ? unitCards(list) : '<div class="card empty">No matches yet. Try “equations”, “graphs”, or “factoring”.</div>'}<div class="notice">First release: the complete equations lesson is ready, alongside the original topic introductions and exercises. The full course outline is here; additional detailed lessons are still being developed.</div><p class="muted">Course structure follows <a class="text-link" href="https://www.montgomeryschoolsmd.org/curriculum/math/high/algebra1/" target="_blank" rel="noopener">MCPS Algebra 1</a>. Your teacher’s pacing may differ.</p>`;
}
function unit(id) {
  ...
  main.innerHTML = `<h1 class="page-title">Mistakes are part of learning.</h1><p class="intro">Revisit questions that needed another try or a little help. Then practice a fresh example.</p>${
    unique.length
      ? unique
          ...
          .map(
            (a) =>
              `<section class="card" style="margin:12px 0"><h3>${esc(a.question?.prompt || "Equation practice")}</h3><details><summary>See the reasoning</summary><p>${esc(a.question?.solution || "Open practice to work through another example.")}</p></details><a class="text-link" href="#practice/${esc(a.topic)}">Practice this topic →</a></section>`,
          )
          .join("")
...
function route() {
  ...
  document.querySelector("nav").innerHTML = nav
    .map(
      ([id, label]) =>
        `<a href="#${id}" ${page === id ? 'class="active" aria-current="page"' : ""}>${icon(id)}${label}</a>`,
    )
    .join("");
  document.getElementById("breadcrumb").innerHTML =
    `Algebra 1 <span>/</span> ${esc(nav.find((n) => n[0] === page)?.[1] || { unit: "Course", lesson: "Lesson", settings: "Settings", search: "Search" }[page] || "Home")}`;
  (
    ({
      home,
      course,
