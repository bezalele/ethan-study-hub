const STORAGE_KEY = "ethan_math_quest_progress_v1";
...
const topics = [
  {
    ...
    realLife:
      "Budgeting for a gaming headset: if the headset costs $x and your savings are at least $90, then x <= 90.",
    studyPoints: [
    ...
    exercises: [
      {
        ...
        story:
          "Ethan wants a $32 concert ticket and snacks. He has $50 total.",
        prompt: "Write and solve: 32 + s <= 50. What is the maximum snack budget s?",
        answer: "18",
    ...
    exercises: [
      {
        ...
        story:
          "A music app charges $5 base + $2 per premium feature pack.",
    ...
    exercises: [
      {
        title: "Phone Plans",
        story:
          "Plan A: y = 20 + 3x. Plan B: y = 10 + 5x where x is GB used.",
  ...
  },
  {
    ...
    realLife:
      "Savings with compound interest grow exponentially over time.",
    studyPoints: [
    ...
    exercises: [
      ...
      },
      {
        title: "Video Views",
        story:
          "A channel doubles views each day starting at 50 views.",
  ...
  },
  {
    ...
    realLife:
      "A basketball shot arc can be approximated by a parabola.",
    studyPoints: [
  ...
  },
  {
    ...
    exercises: [
      ...
      },
      {
        ...
        story:
          "Team A scores: 10,10,10,10. Team B: 6,10,14,10.",
...
const el = {
  ...
  exerciseMode: document.getElementById("exerciseMode"),
  studyModeBtn: document.getElementById("studyModeBtn"),
  exerciseModeBtn: document.getElementById("exerciseModeBtn"),
...
function wireEvents() {
  ...
  el.exerciseModeBtn.addEventListener("click", () => setMode("exercise"));

  el.difficulty.addEventListener("change", () => {
    if (state.selectedTopicId) {
      renderExerciseMode(getTopicById(state.selectedTopicId));
    }
  });

  el.resetProgress.addEventListener("click", () => {
    if (!confirm("Reset all Math Quest progress?")) {
      return;
    }
...
function renderTopicGrid() {
  ...
  topics.forEach((topic) => {
    ...
    const done = topicProgress.completedIds.length;
    ...
    card.innerHTML = `
      <h3>${topic.title}</h3>
      <p>${topic.bigPicture}</p>
      <div class="topic-meta">
        <span>${done}/${total} complete</span>
        <strong>${pct}%</strong>
      </div>
      <button data-topic-id="${topic.id}">Open Topic</button>
    `;
...
function openTopic(topicId) {
  ...
  el.focusTitle.textContent = `${topic.title} - Ethan's Mission`;
  el.focusPanel.classList.remove("hidden");
  setMode("study");
  renderStudyMode(topic);
  renderExerciseMode(topic);
}
...
function setMode(mode) {
  ...
  el.studyMode.classList.toggle("hidden", !study);
  el.exerciseMode.classList.toggle("hidden", study);
  el.studyModeBtn.classList.toggle("active", study);
  el.exerciseModeBtn.classList.toggle("active", !study);
}
...
function renderStudyMode(topic) {
  ...
  points.className = "study-block";
  points.innerHTML = `<h3>Key Ideas</h3><ul>${topic.studyPoints
    .map((p) => `<li>${p}</li>`)
    .join("")}</ul>`;

  el.studyMode.appendChild(intro);
  el.studyMode.appendChild(points);
}

function renderExerciseMode(topic) {
  el.exerciseMode.innerHTML = "";

  const difficulty = el.difficulty.value;
  const generated = generateBonusExercise(topic, difficulty);
  const allQuestions = [...topic.exercises, generated];

  allQuestions.forEach((question, index) => {
    const node = el.questionTemplate.content.firstElementChild.cloneNode(true);
    const qId = `${topic.id}-${index}`;

    node.querySelector(".question-title").textContent = question.title;
    node.querySelector(".question-story").textContent = `Story: ${question.story}`;
    ...
    solution.textContent = `Steps: ${question.solution}`;
    ...
    node.querySelector(".check-btn").addEventListener("click", () => {
      const ok = isAnswerCorrect(input.value, question.answer);
      feedback.className = `feedback ${ok ? "ok" : "bad"}`;
      feedback.textContent = ok
        ? "Great work. You solved it."
        : "Not quite yet. Use hint or steps, then try again.";

      if (ok) {
        markCompleted(topic.id, qId);
        updateDashboard();
        renderTopicGrid();
      }
    });

    el.exerciseMode.appendChild(node);
  });
}

function isAnswerCorrect(userAnswer, correctAnswer) {
  const cleanUser = userAnswer.trim().toLowerCase().replace(/\s+/g, "");
  const cleanCorrect = String(correctAnswer)
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "");
...
function markCompleted(topicId, questionId) {
  const topicProgress = getTopicProgress(topicId);
  if (!topicProgress.completedIds.includes(questionId)) {
    topicProgress.completedIds.push(questionId);
    state.progress.lastActiveDate = new Date().toISOString().slice(0, 10);
    persistProgress();
  }
}
...
function getTopicProgress(topicId) {
  if (!state.progress.byTopic[topicId]) {
    state.progress.byTopic[topicId] = { completedIds: [] };
  }
  return state.progress.byTopic[topicId];
}

function updateDashboard() {
  const totalBaseExercises = topics.reduce((sum, t) => sum + t.exercises.length + 1, 0);
  const completed = Object.values(state.progress.byTopic).reduce(
    (sum, entry) => sum + entry.completedIds.length,
    0
  );

  const mastery = Math.min(100, Math.round((completed / totalBaseExercises) * 100));

  el.completedCount.textContent = String(completed);
  el.overallMastery.textContent = `${mastery}%`;
  el.overallProgressBar.style.width = `${mastery}%`;

  el.streakCount.textContent = computeStreakLabel(state.progress.lastActiveDate);
  el.confidenceLevel.textContent = confidenceFromMastery(mastery);
}

function computeStreakLabel(lastActiveDate) {
  if (!lastActiveDate) {
    return "0 days";
  }
  ...
  const dayDiff = Math.floor(ms / 86400000);
  ...
  return dayDiff <= 7 ? `${Math.max(1, 7 - dayDiff)} days` : "0 days";
}

function confidenceFromMastery(mastery) {
  if (mastery >= 85) {
...
function generateBonusExercise(topic, difficulty) {
  const bank = {
    easy: {
      title: "Bonus Warm-Up",
      detail: "A quick confidence builder"
    },
    medium: {
      title: "Bonus Challenge",
      detail: "A step-up mission"
    },
    hard: {
      title: "Boss Level",
      detail: "A deeper reasoning task"
    }
  };

  const set = bank[difficulty];

  return {
    title: `${set.title}: ${topic.title}`,
    story: `Ethan joins a math rescue team. ${set.detail} connected to ${topic.title.toLowerCase()}.`,
    prompt: `In one sentence, explain how ${topic.title.toLowerCase()} helps in real life. Type: real life model`,
    answer: "reallifemodel",
    hint: "Focus on prediction, planning, or decision-making.",
    solution:
      "Any clear statement showing how the topic models real situations is valid. Enter 'real life model' to complete this reflective challenge."
  };
}
