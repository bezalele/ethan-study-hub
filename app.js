...
const foundingPages={
 colonies:{chapter:'CHAPTER 1 OF 5',date:'1607–1775',title:'British Colonies',subtitle:'Before independence, Americans were British colonists.',intro:'The colonies developed local governments and habits of self-rule while remaining under British authority.',question:'How did colonial experience shape American ideas about government?',points:['Colonial assemblies gave many colonists experience with representative government.','Britain still held ultimate authority over the colonies.','Disputes over taxation and representation intensified after the Seven Years’ War.','Colonial resistance gradually shifted from protest toward independence.'],next:'Declaration of Independence',nextAction:"show('declaration')"},
 articles:{chapter:'CHAPTER 3 OF 5',date:'1781–1789',title:'Articles of Confederation',subtitle:'America’s first national government.',intro:'The Articles created a loose union of states with a deliberately limited central government.',question:'Why did the first American system of government struggle?',points:['Each state had one vote in Congress.','Congress could not levy taxes directly.','The national government had limited power over interstate and foreign commerce.','Weaknesses in the system helped lead to the Constitutional Convention.'],next:'Constitutional Convention',nextAction:"openFoundingPage('convention')"},
 convention:{chapter:'CHAPTER 4 OF 5',date:'1787',title:'Constitutional Convention',subtitle:'Designing a stronger — but limited — government.',intro:'Delegates gathered in Philadelphia to revise the Articles and ultimately drafted an entirely new constitutional framework.',question:'How do you give government enough power to work without concentrating too much power?',points:['Delegates debated representation, national power, and the relationship between states and the national government.','The proposed system divided power among legislative, executive, and judicial branches.','Checks and balances were designed so institutions could limit one another.','The Constitution then went to the states for ratification.'],next:'Constitution + Bill of Rights',nextAction:"openFoundingPage('constitution-rights')"},
 'constitution-rights':{chapter:'CHAPTER 5 OF 5',date:'1788–1791',title:'Constitution + Bill of Rights',subtitle:'A new framework — followed by explicit protections for liberty.',intro:'The Constitution established the federal system of government. The first ten amendments became the Bill of Rights.',question:'How does the system balance government power with individual liberty?',points:['The Constitution establishes national institutions and divides governmental power.','Federalism divides authority between national and state governments.','Ratification produced major debate over the scope of national power.','The Bill of Rights protects specific liberties and limits government action.'],next:'Foundations Course Area',nextAction:'openLearn(1)'}
};
function openFoundingPage(key){
 const x=foundingPages[key], el=document.getElementById('foundingDetailContent'); if(!x||!el)return;
 el.innerHTML='<div class="fd-hero"><button onclick="show(\'home\')">← Founding Journey</button><small>'+x.chapter+'</small><div class="fd-date">'+x.date+'</div><h1>'+x.title+'</h1><h2>'+x.subtitle+'</h2><p>'+x.intro+'</p></div><div class="fd-body"><section><p class="eyebrow">THE BIG PICTURE</p><h2>'+x.question+'</h2><p>This page is designed to give you the story first. Open the ideas below as you study.</p><div class="fd-points">'+x.points.map((p,i)=>'<details '+(i===0?'open':'')+'><summary><b>0'+(i+1)+'</b>'+p.split('.')[0]+'<span>+</span></summary><p>'+p+'</p></details>').join('')+'</div></section><aside><p class="eyebrow">KEY TAKEAWAYS</p>'+x.points.map((p,i)=>'<div><b>'+(i+1)+'</b><span>'+p+'</span></div>').join('')+'</aside></div><div class="fd-next"><button onclick="show(\'home\')">← Back to journey</button><button onclick="'+x.nextAction+'">Next: '+x.next+' →</button></div>';
 show('foundingDetail');
}
const learnChapters = {
