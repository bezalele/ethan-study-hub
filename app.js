...
function renderCourseMap(selected=1){
 ...
 ? '<div class="gov-structure"><div class="gov-top"><b>U.S. CONSTITUTION</b><span>Creates three separate branches</span></div><div class="gov-branches"><article><strong>LEGISLATIVE</strong><h4>Congress</h4><p>Makes laws</p><small>House + Senate</small></article><i>⇄</i><article><strong>EXECUTIVE</strong><h4>President</h4><p>Executes laws</p><small>Departments + agencies</small></article><i>⇄</i><article><strong>JUDICIAL</strong><h4>Federal Courts</h4><p>Interprets laws</p><small>Supreme Court + lower courts</small></article></div><div class="gov-checks">Checks & balances: each branch has powers that can limit the others.</div></div>'
 : selected===1
 ...
 : selected===4
 ? '<div class="summary-chain"><span>Family & community</span><i>→</i><span>Political socialization</span><i>→</i><span>Public opinion</span><i>→</i><span>Polling</span></div>'
 : '<div class="summary-chain"><span>Citizens</span><i>→</i><span>Parties & groups</span><i>→</i><span>Campaigns & media</span><i>→</i><span>Voting & participation</span></div>';
 el.innerHTML='<div class="cm-explorer"><aside class="cm-list"><p class="eyebrow">THE COURSE · FIVE AREAS</p>'+courseAreas.map(x=>'<button class="'+(x.n===selected?'active':'')+'" onclick="renderCourseMap('+x.n+')"><span>0'+x.n+'</span><div><small>UNIT '+x.n+' · '+x.weight+'</small><b>'+x.short+'</b><p>'+x.title+'</p></div><i>›</i></button>').join('')+'</aside><section class="cm-summary clean"><header><small>UNIT 0'+u.n+' · COURSE SUMMARY</small><h2>'+u.title+'</h2><p>'+u.q+'</p></header><div class="cm-summary-body"><div class="cm-big"><p class="eyebrow">THE BIG PICTURE</p><h3>'+u.summary+'</h3><p>'+u.fact+'</p></div><div class="cm-topic-preview"><p class="eyebrow">WHAT YOU’LL STUDY</p>'+u.topics.map((t,i)=>'<button onclick="openUnitStudy('+u.n+')"><span>0'+(i+1)+'</span><b>'+t+'</b><i>→</i></button>').join('')+'</div><section class="cm-visual"><p class="eyebrow">SEE IT SIMPLY</p>'+visual+'</section><button class="cm-enter" onclick="openUnitStudy('+u.n+')">Open the full '+u.short+' study page →</button></div></section></div>';
}
