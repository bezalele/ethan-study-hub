...
function renderCourseMap(selected=1){
 ...
 const visuals=[
  ['https://upload.wikimedia.org/wikipedia/commons/9/9d/Scene_at_the_Signing_of_the_Constitution_of_the_United_States.jpg','Founding the system'],
  ['https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/United_States_Capitol_west_front_edit2.jpg/640px-United_States_Capitol_west_front_edit2.jpg','Institutions & power'],
  ['https://commons.wikimedia.org/wiki/Special:Redirect/file/US%20Bill%20of%20Rights.jpg?width=700','Rights & liberties'],
  ['https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Vote_here_sign.jpg/640px-Vote_here_sign.jpg','Beliefs & opinion'],
  ['https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Voters_at_the_voting_booths.jpg/640px-Voters_at_the_voting_booths.jpg','Participation']
 ];
 el.innerHTML='<div class="cm-explorer"><aside class="cm-list"><p class="eyebrow">THE COURSE · FIVE AREAS</p>'+courseAreas.map(x=>'<button class="'+(x.n===selected?'active':'')+'" onclick="renderCourseMap('+x.n+')"><span>0'+x.n+'</span><div><small>UNIT '+x.n+' · '+x.weight+'</small><b>'+x.short+'</b><p>'+x.title+'</p></div><i>›</i></button>').join('')+'</aside><section class="cm-summary"><div class="cm-summary-image" style="background-image:linear-gradient(90deg,#07100ed9,#07100e2b),url(\''+visuals[selected-1][0]+'\')"><small>UNIT 0'+u.n+' · COURSE SUMMARY</small><h2>'+u.title+'</h2><p>'+u.q+'</p></div><div class="cm-summary-body"><div><p class="eyebrow">THE BIG PICTURE</p><h3>'+u.summary+'</h3><p>'+u.fact+'</p></div><div class="cm-topic-preview"><p class="eyebrow">WHAT YOU’LL STUDY</p>'+u.topics.map((t,i)=>'<button onclick="openUnitStudy('+u.n+')"><span>0'+(i+1)+'</span><b>'+t+'</b><i>→</i></button>').join('')+'</div><div class="cm-path"><p class="eyebrow">THE LEARNING PATH</p><div>'+u.flow.slice(0,5).map((x,i)=>'<span><b>'+(i+1)+'</b>'+x+'</span>').join('<i>→</i>')+'</div></div><button class="cm-enter" onclick="openUnitStudy('+u.n+')">Open the full '+u.short+' study page →</button></div></section></div>';
}
